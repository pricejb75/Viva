...
public class JdbcWishlistDao implements WishlistDao {
    ...
    public Wishlist getWishlist(int userId, int id) {
        String sql =
            """
                SELECT
                    w.*, wi.wishlist_item_id, wi.product_id
                FROM
                    wishlist w LEFT JOIN wishlist_item wi ON (w.wishlist_id = wi.wishlist_id)
                WHERE
                   w.wishlist_id = ? AND w.user_id = ?
            """;
        ...
        String sql =
                """ 
                    ...
                    WHERE
                        wishlist_id = ?
                    AND
                        wishlist_id IN (SELECT wishlist_id FROM wishlist WHERE user_id = ?); 
                    DELETE FROM
                        wishlist
                    WHERE
                        wishlist_id = ? AND user_id = ?      
                 """;
    ...
    public int addItem(WishlistItem wishlistItem) {
        ...
        int newId =
                jdbcTemplate.queryForObject(sql, int.class, wishlistItem.getWishlistId(), wishlistItem.getProductId());
        return newId;
    }
